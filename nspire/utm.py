...
def is_valid_zone_letter(zone_letter):
    zone_letter = zone_letter.upper()
    if not 'C' <= zone_letter <= 'X' or zone_letter in ['I', 'O']:
        raise OutOfRangeError('zone letter out of range (must be between C and X)')


def is_valid_zone_number(zone_number):
    if not 1 <= zone_number <= 60:
        raise OutOfRangeError('zone number out of range (must be between 1 and 60)')


def is_valid_zone(zone_number, zone_letter):
    is_valid_zone_number(zone_number)
    if zone_letter:
        is_valid_zone_letter(zone_letter)
...
def to_latlon( easting, 
    ...
    is_valid_zone(zone_number, zone_letter)
...
def from_latlon(latitude, longitude, force_zone_number=None, force_zone_letter=None, force_northern=None):
    ...
    if force_zone_number is not None:
        is_valid_zone(force_zone_number, force_zone_letter)
